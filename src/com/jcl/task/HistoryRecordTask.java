...
public class HistoryRecordTask {
	...
	Map<String,Subzh> storeSubzhs=new HashMap<String,Subzh>();
	 ...
	 @Scheduled(cron="00 55 08 * * ? ")   //早上5点半点执行操作 
	 public void aTask(){  
		 ...
		 try {
			 ...
			 if(isstartcj<1 && isstartwt<1){
				 ...
				 String changeDay = DateUtil.changeDay(tradeday, "yyyy-MM-dd", -1);
				 Integer intiday=Integer.parseInt(day2)-1;
				 condition.put("intday", intiday);
				 condition.put("tradedate", changeDay);
				 condition.put("orderday", tradeday);
				 log.info("走线路2");
			 }
			 ...
			 if(isstartcj2>0 || isstartwt2>0){//已经b今天的数据记录了，就不去查
				 return;
			 }
	...
	private void saveDealcj(List<DealCj> cjlist,DBCollection dbCollection){
		try {
			...
			for(DealCj deal:cjlist){
				...
				dbObj.put("username", users.get("username"));//
