...
public interface SubzhMapper {
	...
	@Select("SELECT * FROM subzh WHERE subzh = #{primaryKey}")
	Subzh selectOneByPrimaryKey(String primaryKey);
	
	@Select("SELECT * FROM subzh WHERE allocchannel = #{subzh}")
	List<Subzh> selsectByAllocchannel(String subzh);
	
	@Select("SELECT * FROM subzh WHERE allocagent = #{subzh}")
	List<Subzh> selectByAllocagent(String subzh);

	int insertSubzhObjectSelective(SubzhObject subzh);
	...
	@Select("SELECT * FROM subzh WHERE phone = #{phone}")
	Subzh selectByPhone(String phone);
