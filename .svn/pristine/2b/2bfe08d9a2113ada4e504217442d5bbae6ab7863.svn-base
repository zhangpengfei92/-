...
public class AgentzhfundLogController {
	...
	public String agentzhfundLogList(HttpSession session, Model model) {
		...
		try {
			...
			//List<AgentzhfundLog> userlist = agentzhfundLogservice.selectAll();
			...
			criteria.andBzEqualTo((byte)2);//2是提现
			if(isAdmin!=null && isAdmin>=3){
				
			}
			List<AgentzhfundLog> userlist = agentzhfundLogService.selectByExample(example);
			PageInfo<AgentzhfundLog> userpage = new PageInfo<AgentzhfundLog>(userlist);
			if(userlist != null && userlist.size() > 0){
				...
				/*for(AgentzhfundLog AgentzhfundLog : userlist){
					if(roleFlag){
						for(RightsRole rightsRole : roles){
							if(rightsRole.getId().equals(AgentzhfundLog.getUsertype())){
								AgentzhfundLog.setFuncinfo(rightsRole.getRolename());
								break;
							}
						}
					}
					if(AgentzhfundLog.getStarttime() != null){//起始时间显示
						AgentzhfundLog.setSyncinfo(DateUtil.dateToString(AgentzhfundLog.getStarttime()));
					} else {
						AgentzhfundLog.setSyncinfo("--");
					}
					if(AgentzhfundLog.getEndtime() != null){//结束时间显示
						AgentzhfundLog.setClientfrom(DateUtil.dateToString(AgentzhfundLog.getEndtime()));
					} else {
						AgentzhfundLog.setClientfrom("--");
					}
				}*/
			}
	...
	public String selectPage(String nickname, String usertype, Model model,HttpSession session) throws UnsupportedEncodingException {
		try {
			PageHelper.startPage(PaginationContext.getPageNum(), PaginationContext.getPageSize());
			AgentzhfundLogExample example = new AgentzhfundLogExample();
			String username = (String) session.getAttribute(Constant.SESSION_LOGINNAME);
