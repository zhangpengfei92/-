...
<script type="text/javascript">
	...
	function reinstate(id, stype) {
		window.location.href="${path}/codefeeset/editCodefeeset?id="+id;
	}
	...
	function serach(pageNum) {
		...
		form.action = "${path}/codefeeset/selectPage?pageNum=" + pageNum;
		form.submit();
	}
	...
	function addCodefeeset(){
		window.location.href = "${path}/codefeeset/editCodefeeset";
	}
	
	function addMenu(){
		window.location.href = "${path}/codefeeset/addCodefeesetMenu";
	}
	...
	function deleteUserOne(id) {
		...
		$.messager.confirm('提示', '是否确定删除此记录?', function(yn) {
			if (yn) {
				$.ajax({
					url : "${path}/codefeeset/deleteCodefeeset",
					type : "post",
					data : {
						"id" : id
					},
	...
	function updateAll() {
		...
		layui.use('layer', function() {
			...
			layer.open({
				...
				anim : 0,//动画 
				area : [ '600px', '400px' ],
				content : '${path}/codefeeset/updateCodefeesetMore?codefeesetIds='+cids,
				end : function() {
					//全查询
					window.location.href = "${path}/codefeeset/codefeesetList";
				}
	...
	function deleteMenu(){
		...
		if(schemeid){
			...
			if (yn) {
				$.ajax({
					url : "${path}/codefeeset/deleteCodefeeMenu",
					type : "post",
					data : {
						"id" : schemeid
					},
					...
					success : function(data) {
						if (data == "true") {
							...
							$.messager.alert('提示!', '此方案中有交易用户或有渠道关联，不能删除!', 'error');
						}
					}
				});
			}
...
<body>
	...
	<div class="wrap">
		...
		<div class="Rbox">
            <div class="mainbox">
                 <h3><a href="${path}/codefeeset/codefeesetList" class="on">手续费方案</a></h3>
                 <div class="main">
                      <div class="Inputbox">
		                   ...
		                   <form id="form1" method="post" style="float:left;">
	                           ...
	                           <p><label>市场：</label>
			                         	...
			                         	<option value="15">国际期货交易市场</option>
			                         	<option value="16">香港期货交易所</option>
							      	</select>
	                           </p>
	                           <p><label>合约代码：</label>
                           ...
                           <table width="100%;">
	                      	...
	                      	<c:forEach items="${userlist}" var="user" varStatus="st">
								<tr>
									...
									<td>
										<c:choose>
											<%-- <c:when test="${user.marketcode==3}">中金所CFFEX</c:when>
											<c:when test="${user.marketcode==4}">上期所SHFE</c:when>
											<c:when test="${user.marketcode==5}">大商所DCE</c:when>
											<c:when test="${user.marketcode==6}">郑商所CZCE</c:when>
											<c:when test="${user.marketcode==24}">上期能源INE</c:when> --%>
											<c:when test="${user.marketcode==15}">国际期货交易市场</c:when>
											<c:when test="${user.marketcode==16}">香港期货交易所</c:when>
										</c:choose>
									</td>
									<td>${user.code}</td>
									<td>${user.hyname}</td>
									<td><fmt:formatNumber value="${user.counterfee1}" pattern="#.##" type="number"/></td>
									<%-- <td><fmt:formatNumber value="${user.counterfee2}" pattern="#.##" type="number"/></td> --%>
									<td><fmt:formatNumber value="${user.pcfee1}" pattern="#.##" type="number"/></td>
									<%-- <td><fmt:formatNumber value="${user.pcfee2}" pattern="#.##" type="number"/></td> --%>
									<td>
										<c:choose>
											<c:when test="${user.bz eq 'CNY'}">人民币</c:when>
											<c:when test="${user.bz eq 'USD'}">美元</c:when>
											<c:when test="${user.bz eq 'EUR'}">欧元</c:when>
											<c:when test="${user.bz eq 'HKD'}">港币</c:when>
										</c:choose>
									</td>
									<td>
