...
function checkData(){
	...
	var isUpdate = 0;//不可修改
		...
		var minus=myDate.getMinutes();     //获取当前分钟数(0-59)
		 if(hour>=16 && hour<=17){//16:00-17:00
	...
	//保存
	 function saveUser() {
		...
		condition.risktype=$("#risktype").val();
		
		if($("#subrisktype").val() !=''  && condition.risktype !=$("#subrisktype").val()){
			if(! checkData()){
				return false;
			};
		}
		...
		<div class="Rbox">		
     		 <div class="mainbox">
                   ...
                   <div class="main">
                      	...
                      	<input type="hidden" value="${sub.risktype}" name="subrisktype" id="subrisktype"/>
                      	<div class="Form">
                           ...
                           <c:if test="${sub.isadmin eq 6 }">
                            ...
                            <div class="dline">
                                 <span style="color:red;margin-left:60px;">*归属渠道、代理商，经纪人为非必填。如不填，则该用户归属平台。</span>
                            </div>
                            </c:if>
                           ...
                           <c:if test="${sub.isadmin eq 6 }">
                             <div class="dline">
                                 ...
                                 <div id="parentart" class="dselect">
			                        ...
			                        <c:if test="${isholder eq 1}">
				                        ${sub.bdtd}<input type="hidden" value="${sub.bdtd}" id="bdtd" name="bdtd" />
			                        </c:if>
			                    </div>
                            </div>
                            <div class="dline">
                            ...
                            <div class="dline">
                                <label>手续费分成方案方案：</label>
			                    <div id="parentart" class="dselect">
			                        <select name=risktype id="risktype">
			                        <option value="0" >请选择</option>
			                         	<c:forEach items="${feetemplate_menuList}" var="sn">
			                         		<option value="${sn.id}" <c:if test="${sn.id eq sub.risktype }">selected</c:if>>${sn.title}</option>
			                         	</c:forEach>
			                        </select>
			                    </div>
                            </div>
