...
<script type="text/javascript">
	...
	function deleteSubzh(isadmin,id) {
		$.messager.confirm('提示', '是否确定删除此经纪人?', function(yn) {
			if (yn) {
				$.ajax({
					...
					success : function(data) {
						if (data.status == 0) {
							$.messager.alert('提示!', '成功删除此经纪人!', 'info',
							function() {
								parent.RefreshPage();
							});
						}else{
	...
	function setper(isadmin,subzh) {
		...
		$.post("${path}/fund/ispre?subzh="+subzh+"&isadmin="+isadmin,function(d){
			if(d=="true"){
	...
	function serach(pageNum) {
		...
		form.action = "${path}/fund/subzhList?pageNum=" + pageNum+"&isadmin=5";
		form.submit();
	}
	//翻页
	function tzfy() {
	...
	function RefreshPage() {
		window.location.href = "${path}/fund/subzhList?isadmin=5";
	}
	...
	var qrcode="";
	//查看某个经纪人的二维推广码
	function seeTheCode(manage,pt,channel,agent,broker){
		...
		if(!manage || !pt || !channel || !agent){
			$.messager.alert('提示!', '此经纪人的平台有问题不能用于推广!', 'info');
			$('#Tindexcode').fadeOut();
			$('#Tindexcode').hide();
			return false;
		}
		var url=codeurl+"?manage="+manage+"&allocpt="+pt+"&pagentzh="+broker+"&allocchannel="+channel+"&allocagent="+agent+"&allocbroker="+broker;
		//生成二维码
		...
		<div class="Rbox">
            <div class="mainbox">
                  <ul>
		            ...
		            <c:if test="${sessionScope.SESSION_ISADMIN lt 5 }"><li class="selectchx" onclick="change(5)" id="entrustlable">经纪人账户</li></c:if>
                      ...
                      <div class="Inputbox">
		                   <form id="form1" method="post" style="float:left;">
	                           <p>
			                        ...
			                        <select name="allocagent" id="allocagent" class="selectyz">
			                         	<option value="">==请选择==</option>
	                           ...
	                           <p style="margin-top:10px;"><label>经纪人编号：</label><input type="text" id="subzh" name="subzh" value="${param.subzh}" placeholder="请输入经纪人编号"/>
	                           		<label>经纪人名称：</label><input type="text" id="name" name="name" value="${param.name}" placeholder="请输入经纪人名称"/>
	                           </p>
	                           <p>
                           ...
                           <button class="cx xbuild" onclick="findsubzh(5,'');" style="width:110px;">新增经纪人</button>
                           </p>
                      </div>
                    <div class="mtable">
                           <table width="100%;">
	                      	...
	                      	<c:forEach items="${userlist}" var="user" varStatus="st">
	                      		<tr>
									...
									<td>${user.phone}</td>
									<td>
									<a href="javascript:;" onclick="setper(5,this.id)" id="${user.subzh}" style="padding-right: 0px;">设置权限</a>
									<em>|</em><a href="javascript:;" onclick="findsubzh(5,this.id)" id="${user.subzh}" style="padding-right: 0px;">修改</a>
									<em>|</em><a href="javascript:;" onclick="resetPass(this.id)" id="${user.subzh}" style="padding-right: 0px;">密码初始化</a>
									<em>|</em><a href="javascript:;" onclick="deleteSubzh(5,this.id)" id="${user.subzh}" style="padding-right: 0px;">删除</a>
									<em>|</em><a href="javascript:;" onclick="seeTheCode('${user.manage}','${user.allocpt}','${user.allocchannel}','${user.allocagent}','${user.subzh}')" 
