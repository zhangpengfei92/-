...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			ExpandColumn: null,
			tree_root_level : 0,
			prmNames: {page:"page",rows:"rows", sort: null,order: null, search:null, nd:null, id:"id",oper:"oper",editoper:"edit",addoper:"add",deloper:"del", subgridid:"id", npage: null, totalrows:"totalrows"},
			forceFit : false,
			gridstate : "visible",
			cellEdit: false,
			cellsubmit: "remote",
		...
		updatepager = function(rn, dnd) {
			...
			if(pgboxes) {
				...
				if(ts.p.pginput===true) {
					...
					var strButton = "";
					var preButton = "";
					if(ts.p.lastpage*1 > ts.p.page){
						var preInt = (ts.p.lastpage*1 - ts.p.page);//10-3 = 7
						preInt = 9 - preInt;//1
						if(preInt > 0){//补充条数
							var startInx = ts.p.lastpage*1 - 9;
							for(var preInx = 1; preInx < preInt; preInx++){
								if((startInx+preInx) > 0){
									preButton = preButton + " " + "<input class='ui-pg-input-no' type='button' value='"+(startInx+preInx)+"' role='textbox'/>";
								}
							}
						}
						for(var intInx = 1; intInx < 9; intInx++){
							strButton = strButton + " " + "<input class='ui-pg-input-no' type='button' value='"+(ts.p.page*1+intInx)+"' role='textbox'/>";
							if((ts.p.page*1+intInx) == ts.p.lastpage*1){
								intInx = 10;
							}
						}
						$(sppg).html(strButton);
						$("#sp_1_gridPage_preStart").html(preButton);
						$("#sp_1_gridPage_preno").html("");
						$('input.ui-pg-input-no').click( function(e) {
							ts.p.page = ($(this).val()>0) ? ($(this).val()>ts.p.lastpage?ts.p.lastpage:parseInt($(this).val())):ts.p.page;
							populate();
							return false;
						});
					} else if(ts.p.lastpage*1 == ts.p.page){
						if(ts.p.page*1 > 1){
							for(var intInx = 1; intInx < 9; intInx++){
								if((ts.p.page*1-9+intInx) > 0){
									strButton = strButton + " " + "<input class='ui-pg-input-no' type='button' value='"+(ts.p.page*1-9+intInx)+"' role='textbox'/>";
									if((ts.p.page*1-intInx) == 1){
										intInx = 10;
									}
								}
							}
						}
						$("#sp_1_gridPage_preno").html(strButton);
						$("#sp_1_gridPage_preStart").html("");
						$(sppg).html("");
						$('input.ui-pg-input-no').click( function(e) {
							ts.p.page = ($(this).val()>0) ? ($(this).val()>ts.p.lastpage?ts.p.lastpage:parseInt($(this).val())):ts.p.page;
							populate();
		...
		setPager = function (pgid, tp){
			...
			pginp = "",
			pgl="<table cellspacing='0' cellpadding='0' border='0' style='table-layout:auto;float:right;' class='ui-pg-table'><tbody><tr>",
			...
			$("#"+$.jgrid.jqID(pgid) )
			.append("<div id='"+pgcnt+"' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;height:100%;' role='row'><tbody><tr><td id='"+lft+"' align='left'></td><td id='"+rgt+"' align='center' style='white-space:pre;display:none;'></td><td id='"+cent+"' align='right'></td></tr></tbody></table></div>")
			...
			if(dir=="rtl") { pgl += str; }
			if(ts.p.pginput===true) { 
				//alert(ts.p.pgtext + "*" + $.jgrid.jqID(pgid));
				pginp= "<td dir='"+dir+"'><span id='sp_1_gridPage_preno'></span><span id='sp_1_gridPage_preStart'></span>"+$.jgrid.format(ts.p.pgtext || "",
					"<input class='ui-pg-input' type='button' size='2' maxlength='7' value='0' role='textbox'/>","<span id='sp_1_"+$.jgrid.jqID(pgid)+"'></span>")+"</td>";}
			if(ts.p.pgbuttons===true) {
				var po=["first"+tp,"prev"+tp, "next"+tp,"last"+tp]; if(dir=="rtl") { po.reverse(); }
				//pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon ui-icon-seek-first'></span></td>";
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all' style='background:none;'><span class='ui-icon ui-icon-seek-prev'>上一页</span></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all' style='background:none;'><span class='ui-icon ui-icon-seek-next'>下一页</span></td>";
			...
			if(twd > 0) {
				...
				$("td#"+pgid+"_"+ts.p.pagerpos,"#"+pgcnt).width(twd+200);
			}
			ts.p._nvtd = [];
			...
			if(ts.p.pginput===true) {
				$('input.ui-pg-input').click( function(e) {
