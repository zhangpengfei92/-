...
<script type="text/javascript">
	...
	function buy(){
		...
		if(bFlag){
			var txmoney = $.trim($("#txmoney").val());
			 window.location = "${path}/subzhfund/payMent?username="+username+"&txmoney="+txmoney+"&channel=union";
		}
	}
...
/* 	function xfjl(){
	...
	function checkNum(num){
		...
		if(isNaN(num)||num<1.0){
			...
			$.messager.alert('提示',"充值金额必须为数字,且金额必须大于1.0",'info');
                        ...
                        <form action="" id="infoInputForm" method="post">
                            <ul>
                                <li>
                                    ...
                                    <p class="icontit2">充值金额：<input type="text" id="txmoney" name="txmoney" value="" placeholder="请输入充值金额" 
                                    onkeyup="checkNum(this.value)" onpaste="checkNum(this.value)"/>元</p>
                                    <span></span></div>
                                </li>
                            </ul>
             				<div class="paymon">
