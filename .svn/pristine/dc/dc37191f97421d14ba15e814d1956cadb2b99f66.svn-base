...
public class StringUtil {
	 ...
	 * @return
	 */
	public static String getElement(String strs,String str){
		Gson gson = new Gson();
		String param = "";
		try {
			param = java.net.URLDecoder.decode(strs, "utf-8");
		} catch (UnsupportedEncodingException e) {
			e.getMessage();
		}
		byte[] base64 = Base64.decode(param);
		JsonParser parser = new JsonParser();
		JsonElement jelement = parser.parse(new String(base64));
		JsonObject jobj = jelement.getAsJsonObject();
		JsonElement qlistelement = jobj.get(str);
		Type type = new TypeToken<String>(){}.getType();
		String element = gson.fromJson(qlistelement, type);
		return element;
	}
	/**
	 * 判断字符串是否为null和空
	...
	public static String getPinyin(String name){
		...
		hy.setToneType(HanyuPinyinToneType.WITHOUT_TONE);// 没有音调数字
		String b = "";
		if(!StringUtil.isAllNullOrEmpty(name)){
			char c[] =  name.toCharArray();
			String[][] a = new String[name.length()][];
			try {
				for(int i = 0; i < c.length; i++){
					String str = PinyinHelper.toHanyuPinyinStringArray(c[i],hy)[0];
