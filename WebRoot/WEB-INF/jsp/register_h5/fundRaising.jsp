...
<script type="text/javascript">
	function buy(){
		...
		if(!isNaN(amount) && Number(amount)<=Number(minmoney)){
			openwindows("提现金额必须大于等于"+minmoney)
			return false;
		}
		
		if(!isNaN(amount) && Number(amount)>=Number(maxmoney)){
	...
	//根据下拉列表中的银行编号获取开户银行名称
 	 ...
 	 if(banchname==""||banchname.length<=0){
    	 openwindows("请输入开户分行")
		 bFlag = false;
         return false;
     }
				 ...
				 console.log(msg);
		 		 if(msg=="ok"){
					 layui.use('layer',function(){
							...
							//3.调用layer模块
							layer = layui.layer;
					 		layer.confirm(msg+"--提现申请提交成功", { btn : [ '确定'],closeBtn : 0}, function(index){							  
					 ...
					 layui.use('layer',function(){
							...
							//3.调用layer模块
							layer = layui.layer;
					 		layer.confirm(msg+"--提现申请提交失败", { btn : [ '确定'],closeBtn : 0}, function(index){							  
						 		window.location.href="${path}/subzhfund/stockoptionJump"
						});
					 });
	...
	/* 			  if(msg=="ok"){
						 layer.confirm('提现申请提交成功', { btn : [ '确定']},function(index){
						 layer.close(index);
						 window.location.href="${path}/subzhfund/stockoptionJump";
						 }) 
					 }else{
						 layer.confirm('提现申请提交失败', { btn : [ '确定']},function(index){
							 layer.close(index);
							 window.location.href="${path}/subzhfund/stockoptionJump";
							}) 
...
<body class="grey">
    ...
    <section class="phonewrap2">
           <div class="yue">
                   <p class="num">账户资产：</p>
                   <p><fmt:formatNumber value="${sub.fundbalance==null?0:sub.fundbalance}" pattern="0.00"/>  元（人民币）</p></p>            
                    <input type="hidden" id="fund" value="<fmt:formatNumber value="${sub.fundbalance==null?0:sub.fundbalance}" pattern="0.00"/>">
				  	<input type="hidden" id="maxmoney" value="<fmt:formatNumber value="${fundset.maxmoney==null?0:fundset.maxmoney}" pattern="0.00"/>">
					<input type="hidden" id="minmoney" value="<fmt:formatNumber value="${fundset.minmoney==null?0:fundset.minmoney}" pattern="0.00"/>">
             </div>
        <div class="paybox">     
		<div class="paybox2">
                  <h3>提现</h3>
                       		...
                       		<ul>
								...
								<div class="line2"></div>
                                 ...
                                 <li>
                                    ...
                                    <input id="banchName" name="banchName" value="" placeholder="请输入银行卡开户支行名称" type="text">
                                    </p><span></span></div>
                                </li>
                                <div class="line2"></div>
                                 <li>
                                    ...
                                    <p class="icontit2">收款人姓名：
                                    	<input id="name" name="name" value="" placeholder="请输入收款人姓名" type="text">
                                    </p><span></span></div>
                                </li>
                                <div class="line2"></div>
                                 <li>
                                    ...
                                    <input id="bankCardNumber" name="bankCardNumber" value="" placeholder="收款人银行账号" type="text"></p>
                                    <span></span></div>
                                </li>
                                <div class="line2"></div>
                                   <li>
                                    <div class="payicon wpayicon"><p class="icon02"></p>
                                    <p class="icontit2">预留手机号：<input id="phoneNum" name="phoneNum" value="" placeholder="预留手机号" type="text">
                                    </p><span></span></div>
                                </li>
                                <div class="line2"></div>
                                   <li>
                                    ...
                                    <input id="idcard" name="idcard" value="" placeholder="请输入身份证号" type="text">
                                    </p><span></span></div>
                                </li>
                            	<div class="line2"></div>
                            	<li>
