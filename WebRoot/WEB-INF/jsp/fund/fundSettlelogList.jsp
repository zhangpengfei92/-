...
<script type="text/javascript">
	...
	$(function(){
	     ...
	     var html='';
		$.post("${path}/fundPzlog/getCountFundSettlelog",formdate,function(data){
			if(data !=null ){
				html=html+"<td>合计:</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td>"+data.summoney+"</td><td>"+data.sumfeenextbalance+"</td><td>"+data.sumaddbalance+"</td><td>"+data.sumoverbalance+"</td>";
			}else{
                 ...
                 <div class="main">
                      <div class="Inputbox">
		                   <form id="form1" method="post" style="float:left;">
		                   	   <p><label>用户账户：</label><input type="text" id="subzh" name="subzh" value="${trade.subzh}" placeholder="请输入用户账户"/>
		                   	   <label>所属渠道：</label>	
			                        <select name="channel" id="channel" onchange="getSubDataByQd(this.value)" class="selectyz">
	                           			<option value="">==请选择==</option>
			                         	<c:forEach items="${channelList}" var="channel">
			                         		<option value="${channel.subzh}" <c:if test="${trade.channel eq channel.subzh }">selected</c:if> >${channel.subzh}-${channel.name}</option>
			                         	</c:forEach>
			                        </select>
	                           ...
	                           <p>
			                        ...
			                        <select name="agentzh" id="agentzh" onchange="getSubDataByDls(this.value)" class="selectyz">
			                         	...
			                         	<c:forEach items="${agentList}" var="agent">
			                         		<option value="${agent.subzh}" <c:if test="${trade.agentzh eq agent.subzh }">selected</c:if> >${agent.subzh}-${agent.name}</option>
			                         	</c:forEach>
			                        </select>
	                           		<label>所属经纪人：</label>
	                           		<select name="broker" id="broker" class="selectyz">
			                         	...
			                         	<c:forEach items="${brokerList}" var="sn">
			                         		<option value="${sn.subzh}" <c:if test="${trade.broker eq sn.subzh }">selected</c:if>>${sn.subzh}-${sn.name}</option>
			                         	</c:forEach>
			                        </select>
	                           </p>
	                           <p style="padding-top:5px;"><label>合约代码：</label><input type="text" id="stockCode" name="stockCode" value="${trade.stockCode}" placeholder="请输入" style="width:150px;"/></p>
	                           <p style="padding-top:5px;">
	                           		<label class="wid">成交时间：</label>
			        				<span><input id="starttime" name="starttime" type="text" class="input_text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'%y-%M-%d'})" value="${trade.starttime}" style="width:125px;min-width:125px;margin-left:5px;margin-right:0px;"/>
			        				<span style="margin-top:4px;">&nbsp;&nbsp;至&nbsp;&nbsp;</span>
			        				<input id="endtime" name="endtime" type="text" class="input_text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'%y-%M-%d'})" value="${trade.endtime}" style="width:125px;min-width:125px;"/></span>
	                           </p>
	                           <p style="padding-top:5px;">
