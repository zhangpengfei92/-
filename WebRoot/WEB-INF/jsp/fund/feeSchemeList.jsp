<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title>收费方案</title>
<link rel="stylesheet" type="text/css" href="${path}/css/style.css" />
	<script type="text/javascript" src="${path}/js/jquery-1.11.3.min.js"></script>
	<!-- 引入默认css -->
	<link rel="stylesheet" type="text/css" href="${path}/js/easyui/themes/default/easyui.css" />
	<link rel="stylesheet" type="text/css" href="${path}/js/easyui/themes/icon.css" />
	<script type="text/javascript" src="${path}/js/easyui/jquery.min.js"></script>
	<script type="text/javascript" src="${path}/js/easyui/jquery.easyui.min.js"></script>
	<script type="text/javascript" src="${path}/js/easyui/locale/easyui-lang-zh_CN.js"></script>
</head>
<script type="text/javascript">
	$(function(){
		//$("#left_a_2").addClass('on');
		var feeschemeIdValue = $("#feeschemeIdValue").val();
		if(feeschemeIdValue != null && feeschemeIdValue != ""){
			$("#feeschemeId").val(feeschemeIdValue);
		}
	});
	
	/* 全选和反选 */
	var Acheckbox = false;
	function checkAll() {
		if (Acheckbox) {
			//选择每个被选中的input
			$(":checkbox").each(function() {
				this.checked = false;
			});
			Acheckbox = false;
		} else {
			$(":checkbox").each(function() {
				this.checked = true;
			});
			Acheckbox = true;
		}
	}

	//通过
	function reinstate(id, stype) {
		var myDate = new Date();
		var week=myDate.getDay();//获取当前星期X(0-6,0代表星期天)
		var istradeDay=0;//0不是交易日。1是交易日
		var isUpdate = 0;//不可修改
		if(week!=0&&week!=6){
			var hour=myDate.getHours();       //获取当前小时数(0-23)
			var minus=myDate.getMinutes();     //获取当前分钟数(0-59)
			if(hour>=13 && hour<15){//13:00-15:00
				istradeDay=1;
			}else if(hour==10){//10:00-11:00
				istradeDay=1;
			}
			if(hour==9 && minus>=30){//09:30-10:00
				istradeDay=1;
			}
			if(hour > 14 && hour < 16){
				isUpdate = 1;//可修改  15:00-16:30
			} 
			if(hour == 16 && minus < 30){
				isUpdate = 1;//可修改
			}
		} else {
			isUpdate = 1;//可修改
		}
		if(isUpdate){
			window.location.href="${path}/cost/editfeeScheme?id="+id+"&istradeDay="+istradeDay+"&type="+stype;
		} else {
			alert("交易日当前时间不可修改。");
		}
	}
	
	//查询
	function serach(pageNum) {
		if (pageNum == 0) {
			pageNum = 1;
		}
		var form = document.getElementById("form1");
		form.action = "${path}/cost/selectPageFeeSchemeList?pageNum=" + pageNum;
		form.submit();
	}
	
	//翻页
	function tzfy() {

		var pageNum = $.trim($("#pageNum").val());
		if (!pageNum) {
			$.messager.alert('提示', '请输入页码!', 'info');
			return false;
		}
		var pages = $.trim($("#pages").val());
		if (pageNum*1 > pages*1) {
			$.messager.alert('提示', '输入的页数无效!', 'info'); 
			return false;
		}
		serach(pageNum);
	}

	//刷新页面
	function RefreshPage() {
		window.location.href = "${path}/cost/feeSchemeList";
	}
	
	function setNext(){//重置
		$("#nickname").val("");
	}
	
	function addFeeScheme(){
		window.location.href = "${path}/cost/addfeeScheme";
	}
	
	//修改多条合约信息
	function updateAll() {
		var cids = "";
		var arr = $("input[name='codefeesetId']:checked");
		for (var i = 0; i < arr.length; i++) {
			cids = arr[i].value+","+cids;
		}
		layui.use('layer', function() {
			//2.调用jquery模块
			var $ = layui.jquery,
			//3.调用layer模块
			layer = layui.layer;
			layer.open({
				type : 2,
				title : "修改收费合约规则",
				//不显示关闭按钮  右上角X   1为显示
				closeBtn : 2,
				//遮罩  透明度0-1  屏幕由原始亮度到最黑
				shade : [ 0.5 ],
				//坐标 弹出位置右下角
				//offset: 'rb', //右下角弹出  lt  rt  lb  rb
				anim : 0,//动画 
				area : [ '600px', '400px' ],
				content : '${path}/cost/codefeesetIds?codefeesetIds='+cids,
				end : function() {
					//全查询
					window.location.href = "${path}/cost/codefeesetList";
				}
			});
		}); 
	}
	
</script>
<body>
	<%@include file="../../../head.jsp"%>
	<div class="wrap">
		<%@include file="subzhMaster.jsp"%>
		<div class="Rbox">
            <div class="mainbox">
                 <h3><a href="${path}/cost/feeSchemeList" class="on">收费方案</a></h3>
                 <div class="main">
                      <div class="Inputbox">
                      	   <input type="hidden" id=feeschemeIdValue name="feeschemeIdValue" value="${feeschemeId}"/>
		                   <form id="form1" method="post" style="float:left;">
	                           <p><label>方案名称：</label>
	                           		<select id="feeschemeId" name="feeschemeId">
		                           		<c:forEach items="${feeschemeMenuList}" var="feeschemeMenu">
									    	<option value="${feeschemeMenu.lsh}">${feeschemeMenu.schemename}</option>
								        </c:forEach>
							        </select>
	                           </p>
	                           <p>
	                              &nbsp;&nbsp;<button class="cx" onclick="serach('1')">查询</button>
	                              &nbsp;&nbsp;<button class="cx" onclick="updateAll()">批量修改</button>
	                           </p>
                           </form>
                           <p class="rbutbox">
                           		<button class="cx xbuild" onclick="addFeeScheme();">新建</button>
                           </p>
                      </div>
                    <div class="mtable">
                           <table width="100%;">
	                           <tr class="tit">
								<td width="10px"><input type="checkbox" onclick="checkAll()" /></td>
								<td width="5%">序号</td>
								<td>收费类型</td>
								<td>方案名称</td>
								<td>交易金额比例</td>
								<td>委托方向</td>
								<td>显示状态</td>
								<td width="18%">操作</td>
							</tr>
	                      	<c:forEach items="${userlist}" var="user" varStatus="st">
								<tr>
									<td width="10px"><input type="checkbox" name="codefeesetId" value="${user.id}" /></td>
									<td>${st.count}</td>
									<td>
										<c:choose>
											<c:when test="${user.schemetype==1}">交易佣金</c:when>
											<c:when test="${user.schemetype==2}">印花税</c:when>
											<c:when test="${user.schemetype==3}">过户费</c:when>
											<c:when test="${user.schemetype==5}">管理费</c:when>
										</c:choose>
									</td>
									<td>${user.schemename}</td>
									<td><fmt:formatNumber value="${user.percentage}" pattern="#.##" type="number"/></td>
									<td><c:choose>
											<c:when test="${user.direction=='0,1'}">双向</c:when>
											<c:when test="${user.direction=='0'}">买</c:when>
											<c:when test="${user.direction=='1'}">卖</c:when>
										</c:choose></td>
									<td>
										<c:choose>
											<c:when test="${user.showstatus==1}">显示</c:when>
											<c:when test="${user.showstatus==0}">不显示</c:when>
										</c:choose>
									</td>
									<td>
										<a href="javascript:;" onclick="reinstate('${user.lsh}','${user.schemetype}')" id="${user.lsh}">修改</a>
									</td>
								</tr>
							</c:forEach>
						</table>
		 				</div>
		 				<div class="page">
							<c:choose>
									<c:when test="${userpage.pages==0}">0/${userpage.pages}</c:when>
									<c:otherwise>${userpage.pageNum}/${userpage.pages}</c:otherwise>
								</c:choose> 共<i>${userpage.pages}</i>页
							<a class="bs" href="javascript:;" onclick="serach('1')">首页</a>
							<input type="hidden" value="${userpage.pages}" id="pages" />
							<c:if test="${userpage.pageNum!=1}">
								<a class="bs" href="javascript:;" onclick="serach('${userpage.prePage}')">上一页</a>
							</c:if>
							<c:if test="${userpage.pageNum!=userpage.pages}">
								<a class="bs" href="javascript:;" onclick="serach('${userpage.nextPage}')">下一页</a>
							</c:if>
							<a class="bs" href="javascript:;" onclick="serach('${userpage.pages}')">尾页</a>
							<a ><input type="text" id="pageNum" value="${userpage.pageNum}" style="height: 30px;line-height: 30px;padding: 0 5px;"/></a>
							<a style="cursor: pointer;" type="button" onclick="tzfy()">跳转</a>
						</div>
                    </div>
               </div>
          </div>
     </div>
</body>
</html>