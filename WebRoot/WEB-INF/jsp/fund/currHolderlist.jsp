	...
	<c:if test="${status eq 0 }">
			<div class="mtable">
                <table width="100%;" >
						...
						<input type="hidden"  id="entrust_vol_${st.count}"  name="entrust_vol" value="${holder.sell_vol}">
							...
							<td>${holder.name}</td>
							<td>${holder.agent}</td>
							<td><fmt:formatNumber type="number" value="${holder.position_vol}" maxFractionDigits="0"/></td>
							<c:if test="${empty lsflag or lsflag eq 1}">
								<%-- <td>${holder.history_vol}</td>
								<td>${holder.today_vol}</td> --%>
								<td><fmt:formatNumber type="number" value="${holder.sell_vol}" maxFractionDigits="0"/></td>
							</c:if>
							<td> <fmt:formatNumber type="number" value="${holder.price}" maxFractionDigits="2"/></td>
							<td> <fmt:formatNumber type="number" value="${holder.profit_price}" maxFractionDigits="2"/></td>
							<td> <fmt:formatNumber type="number" value="${holder.loss_price}" maxFractionDigits="2"/></td>
		...
		$(function(){
			...
			$.post("${path}/trade/getTotalHoder",formdate,function(data){
				if(data !=null ){
					html=html+"<td>合计:</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td>"+data.position_vol.toFixed(0)+"</td><td>"+data.sell_vol.toFixed(0)+"</td><td></td><td></td><td></td><td>"+data.profit.toFixed(2)+"</td>";
				}else{
