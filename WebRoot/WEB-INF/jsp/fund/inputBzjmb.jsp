	...
	function saveBzjmb() {
		...
		$.ajax({
	        ...
	        error: function (returndata) {  
	              $.messager.alert('提示!', returndata,'error');
	          }  
		});
	}
	
	function refreshPage() {//取消
		<c:if test="${empty user.tjdtbzj1}">window.location.href="${path}/cost/bzjmbList";</c:if>
		<c:if test="${!empty user.tjdtbzj1}">location.replace(document.referrer);</c:if>
	}
</script>
</head>
<body>
                   ...
                   <form id="bzjmbForm" method="post">
                           ...
                           <div class="dline">
					            <label><font color="red">*</font>合约信息：</label>
							        ...
							        <input type="hidden" id="bbbz2" name="bbbz2" value=""/>
							 </div>
					        <div class="dline">
					            <label>币种</label>
					            <select id="bz" name="bz" class="selectCss">
				                	<option value="CNY" ${user.bz eq 'CNY' ? "selected='selected'":"" }>人名币</option>
				                	<option value="USD" ${user.bz eq 'USD' ? "selected='selected'":"" }>美元</option>
				                	<option value="EUR" ${user.bz eq 'EUR' ? "selected='selected'":"" }>欧元</option>
				                	<option value="HKD" ${user.bz eq 'HKD' ? "selected='selected'":"" }>港币</option>
			      				</select>
					        ...
					        <div class="dline">
					          	<label><font color="red">*</font>收盘冻结保证金(按手数)</label>
					            <input type="text" id="bbbz" name="bbbz" value="${user.bbbz}" onkeyup="value=value.replace(/[^\-?\d.]/g,'')"/>
					        </div>
					        <%-- <div class="dline">
