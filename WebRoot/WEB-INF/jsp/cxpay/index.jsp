...
<script type="text/javascript">
	...
	function buy(){
		...
		var paybtn = $.trim($("input[name='paybtn']:checked").val());
		alert(paybtn)
		if(bFlag){
			$("#paymode").val(paybtn);
				 ...
				 <div class="ls-header-title">充值</div>
                   <div class="paymon" >
						   ...
						   <ul>
							...
							<div class="line"></div>  
							<li>
								<input type="radio" name="paybtn" value="1" checked="checked"/><i class="mtit">网关充值(不支持手机)</i>
								<div class="line"></div>
								<input type="radio" name="paybtn" value="2"/><i class="mtit">支付宝充值</i>
								<div class="line"></div>
								<c:if test="${sub.subzh=='15118953197'}">
									<input type="radio" name="paybtn" value="3"/><i class="mtit">wab快捷支付</i>
								</c:if>	
								<div class="line"></div>
								<input type="radio" name="paybtn" value="4"/><i class="mtit">微信充值</i>						
							</li>
						   </ul>	   
					  </div> 
             			<p><a href="javascript:void(0);" class="consu rebut" onclick="buy()">确认支付</a></p>
             			<a href="http://47.110.37.66/ctpsubcms/index.jsp"></a>
             </div>             
    </section>
    <form id="payfrm" action="${path}/cxpay/paysel" method="post">
    	<input type="hidden" id="subzh" name="subzh" value="${sub.subzh}">
    	<input type="hidden" id="total_fee" name="total_fee" value="">
    	<input type="hidden" id="sdorderno" name="sdorderno" value="${orderno}">
